...
namespace Aden.Util.Common
{
    public class DateTimeHelper
    {
        ...
        /// <returns></returns>
        public static string convertDateTime(string strDt, string strCFormat = "", string strFormat = "")
        {
            DateTime dt = new DateTime();
            //DateTime dt = Convert.ToDateTime(strDt);
            //if (end) dt = dt.AddDays(1).AddSeconds(-1);
            if (string.IsNullOrWhiteSpace(strCFormat))
                dt = Convert.ToDateTime(strDt);
            else
                dt = DateTime.ParseExact(strDt, strCFormat, System.Globalization.CultureInfo.InvariantCulture);
            if (string.Empty.Equals(strFormat))
                strFormat = "yyyy-MM-dd HH:mm:ss.fff";
            return dt.ToString(strFormat);
        }

        public static string convertDateTime(DateTime dt, string strFormat = "")
        {
            //if (end) dt = dt.AddDays(1).AddSeconds(-1);
            if (string.Empty.Equals(strFormat))
                strFormat = "yyyy-MM-dd HH:mm:ss.fff";
